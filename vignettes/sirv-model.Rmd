...
## 6. Using the model in the Shiny application

To explore the model interactively, pass it directly to
`run_epi_app()`. The built-in models remain available alongside yours.

```{r, eval=FALSE}
run_epi_app(models = list(SIRV_MODEL))
```

If you prefer to use only your model:

```{r, eval=FALSE}
run_epi_app(models = list(SIRV_MODEL), only = TRUE)
```

> The SIRV model is also included as a built-in model of the package under
> the name `SIRV_MODEL`, so it also appears by default when calling
> `run_epi_app()` with no arguments.
