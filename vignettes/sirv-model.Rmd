---
title: "SIR model with vaccination (SIRV, Susceptible-Infectious-Recovered-Vaccinated): from demo to vignette"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{SIR model with vaccination (SIRV, Susceptible-Infectious-Recovered-Vaccinated)}
...
Here SIRV stands for Susceptible-Infectious-Recovered-Vaccinated.
...
## 6. Usar el modelo en la aplicación Shiny

Para explorar el modelo de forma interactiva pásalo directamente a
`run_epi_app()`. Los modelos integrados siguen disponibles junto con el tuyo.

```{r, eval=FALSE}
run_epi_app(models = list(SIRV_MODEL))
```

Si prefieres usar únicamente tu modelo:

```{r, eval=FALSE}
run_epi_app(models = list(SIRV_MODEL), only = TRUE)
```

> El modelo SIRV está además incluido como modelo integrado del paquete bajo
> el nombre `SIRV_MODEL`, por lo que también aparece por defecto al llamar
> a `run_epi_app()` sin argumentos.
